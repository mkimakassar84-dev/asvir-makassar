...
function findPiutangByKategoriUmur(message, piutangDetail) {
  ...
  if (!kategori) {
    ...
    const wantsNewest = !wantsOldest && new RegExp(SUPERLATIVE_LOW).test(nMsg) && /piutang|menunggak|tunggak|umur|aging|jatuh\s*tempo/.test(nMsg);
...
- "Frekuensi customer"/"paling sering belanja"/"churn" → "customerInsights" (totalCustomer, totalChurned=tak beli≥60hr, buckets, topByFrekuensi, topBySales).
