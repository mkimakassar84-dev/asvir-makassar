...
// Sheet input bulanan. Kolom G di tiap sheet = "Kode Barang", tempat tim menginput transaksi.
const GIDS_BULANAN = {
  Jan: '0', Feb: '1831734265', Mar: '1586424938', Apr: '1196191226', Mei: '971109321', Jun: '1725396174',
  Jul: '56931099', Agu: '691083737', Sep: '1962720543', Okt: '2035584798', Nov: '468918005', Des: '614537042',
};
...
function findProductSalesBreakdown(message, allTransactions) {
  ...
  // Collect EVERY code mentioned, not just the first. A real reported failure: asked to compare
  // KSFO113 and KSFO128, this returned only KSFO128 and MIRA then reported — repeatedly and
  // confidently — that KSFO113 "has no monthly breakdown", when it has a full one. Any comparison
  // question ("bandingkan X dan Y") is unanswerable while only one side is retrieved.
  ...
  if (!hitKodes.length) return null;

  const perKode = hitKodes.map((kode) => {
    const rows = allTransactions.filter((tx) => tx.kode === kode);
    const monthly = {};
    let totalAmount = 0;
    let totalQty = 0;
    ...
    // Months with no sales are emitted as explicit zeros rather than omitted — a missing month was
    // being read as "data not recorded" and reported to the user as a gap in the system.
    const adaBulan = Object.keys(monthly);
    if (adaBulan.length) {
      const tahun = adaBulan[0].slice(0, 4);
      const bulanTerakhir = nowMakassar().getMonth() + 1;
      for (let b = 1; b <= bulanTerakhir; b++) {
  ...
  return {
    jumlahKode: perKode.length,
    kode: perKode.map((p) => p.kode),
    perKode,
    catatan:
      'SEMUA kode yang disebut user ADA di "perKode" — kalau user membandingkan 2 kode, keduanya ada di sini, JANGAN bilang salah satunya tidak punya data. ' +
      'Tiap "perBulan" sudah lengkap Januari s.d. bulan berjalan; bulan bernilai 0 berarti MEMANG tidak ada penjualan bulan itu, BUKAN "data belum tercatat" — sampaikan sebagai nol, jangan sebagai data hilang. ' +
      'Untuk perbandingan antar PERIODE (mis. Jan-April vs Mei-Agustus), jumlahkan sendiri bulan-bulan yang diminta dari "perBulan" — angkanya sudah tersedia lengkap.',
  };
}
...
function findKodeProdukIssues(message, kodeMentah, allTransactions, auditKolomG) {
  const nMsg = normText(message);
  // "Kolom G" di sheet bulanan adalah kolom Kode Barang, jadi menyebutnya sudah berarti bicara
  // kode barang — "cek kolom G di setiap bulan" harus terpicu walau tidak menyebut "kode barang".
  const sebutKolomG = /kolom\s*g\b/.test(nMsg);
  const bicaraKode = sebutKolomG || /\bsku\b|kode\s*(produk|barang|item|stok|stock)|kode\s*(yang|nya)?\s*(salah|keliru|tidak)/.test(nMsg);
  const bicaraSalah = /salah|keliru|tidak sesuai|ga sesuai|gak sesuai|typo|salah tulis|salah ketik|tidak dikenal|tidak terdaftar|tidak ada di stok|tidak ada di stock|cek penulisan|periksa|audit|sisir/.test(nMsg)
    || (sebutKolomG && /\bcek\b|\bcekkan\b|\bmengecek\b|\bperiksa\b|\blihat\b/.test(nMsg));
  ...
  const mintaTransaksi = /\bsales\b|penjualan|transaksi|grand\s*data|invoice|terjual|penginputan|diinput/.test(nMsg);
  let periksaStok = !mintaTransaksi || mintaStok;
  let periksaTransaksi = !mintaStok || mintaTransaksi;
  // Per bulan: hanya bermakna untuk transaksi — sheet Stock GD MKS tidak menyimpan tanggal input,
  // jadi kesalahan di sana tidak bisa dibagi menurut bulan. Menyebut satu bulan otomatis
  // mempersempit ke transaksi bulan itu.
  const bulanDiminta = extractMonthMention(message);
  const mintaPerBulan = /per\s*bulan|tiap\s*bulan|setiap\s*bulan|dibagi.*bulan|\bbulanan\b|masing[- ]masing bulan/.test(nMsg);
  const mintaKolomG = /kolom\s*g\b|sheet bulanan|setiap sheet|tiap sheet/.test(nMsg);
  if (bulanDiminta || mintaPerBulan || mintaKolomG) { periksaTransaksi = true; if (!mintaStok) periksaStok = false; }
  ...
  if (periksaTransaksi) {
    ...
    for (const t of allTransactions || []) {
      ...
      if (bulanDiminta && !(d && d.getMonth() + 1 === bulanDiminta.month && d.getFullYear() === bulanDiminta.year)) continue;
      if (d) {
        const kunci = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
        if (!bulanMap.has(kunci)) bulanMap.set(kunci, { bulan: `${NAMA_BLN[d.getMonth()]} ${d.getFullYear()}`, kunci, perKode: new Map(), jumlahBaris: 0, nilai: 0 });
        const b = bulanMap.get(kunci);
        b.jumlahBaris += 1;
        b.nilai += toNumber(t.amount);
        // Rincian per kode DI DALAM bulan ini. Tanpa ini MIRA meminjam angka keseluruhan periode
        // ke bagian satu bulan — terbukti: KCO6002 ditulis "4 baris, Rp10.336.000" di bagian Juni,
        // padahal khusus Juni hanya 3 baris, Rp9.576.000.
        if (!b.perKode.has(k)) b.perKode.set(k, { kode: k, jumlahBaris: 0, qty: 0, nilai: 0, contoh: [] });
        const pk = b.perKode.get(k);
        pk.jumlahBaris += 1;
        pk.qty += toNumber(t.qty);
        pk.nilai += toNumber(t.amount);
        if (pk.contoh.length < 2) pk.contoh.push({ tanggal: t.tanggal, invoice: t.invoice, customer: t.customer });
      }
    ...
    perBulan = [...bulanMap.values()]
      .sort((a, b) => a.kunci.localeCompare(b.kunci))
      .map((b) => ({
        bulan: b.bulan,
        jumlahKode: b.perKode.size,
        jumlahBaris: b.jumlahBaris,
        nilai: b.nilai,
        rincian: [...b.perKode.values()]
          .sort((x, y) => y.nilai - x.nilai)
          .map((x) => ({ ...x, kemungkinanMaksudnya: usulanKodeMirip(x.kode, kodeAda) })),
      }));
  }

  // Per bulan dari KOLOM G sheet bulanan, bila hasil pemeriksaan sinkronisasi tersedia — isinya
  // sama dengan yang terbaca lewat Grand Data (terbukti identik, 30 lawan 30), tapi membawa sheet
  // dan nomor baris sehingga langsung bisa dibetulkan di tempat datanya diinput.
  let perSheetKolomG = null;
  if (periksaTransaksi && auditKolomG && Array.isArray(auditKolomG.temuan) && (bulanDiminta || mintaPerBulan || mintaKolomG)) {
    const URUT = Object.keys(GIDS_BULANAN);
    const tahunIni = nowMakassar().getFullYear();
    const lengkap = { Jan: 'Januari', Feb: 'Februari', Mar: 'Maret', Apr: 'April', Mei: 'Mei', Jun: 'Juni', Jul: 'Juli', Agu: 'Agustus', Sep: 'September', Okt: 'Oktober', Nov: 'November', Des: 'Desember' };
    perSheetKolomG = URUT
      .filter((s, idx) => !bulanDiminta || (bulanDiminta.month === idx + 1 && bulanDiminta.year === tahunIni))
      .map((s) => {
        const ts = auditKolomG.temuan.filter((t) => t.sheet === s);
        const perKode = new Map();
        for (const t of ts) {
          const k = String(t.kode).trim();
          if (!perKode.has(k)) perKode.set(k, { kode: t.kode, masalah: t.masalah, jumlahBaris: 0, lokasi: [] });
          const e = perKode.get(k);
          e.jumlahBaris += 1;
          e.lokasi.push({ baris: t.baris, invoice: t.invoice, tanggal: t.tanggal, customer: t.customer });
        }
        const diperiksa = (auditKolomG.diperiksaPerSheet || {})[s] || 0;
        return {
          sheet: s,
          bulan: `${lengkap[s]} ${tahunIni}`,
          barisDiperiksa: diperiksa,
          status: !diperiksa ? 'belum ada input' : ts.length ? 'ada salah input' : 'bersih',
          jumlahBarisSalah: ts.length,
          rincian: [...perKode.values()].map((e) => ({ ...e, kemungkinanMaksudnya: usulanKodeMirip(e.kode, kodeAda) })),
        };
      });
  }
  ...
  return {
    ...
    kodeTransaksiTidakAdaDiStok: tidakDikenal ? {
      periode: bulanDiminta ? `${NAMA_BLN[bulanDiminta.month - 1]} ${bulanDiminta.year}` : 'seluruh data yang tersinkron',
    ...
    kolomGSheetBulanan: perSheetKolomG,
    catatanKolomG: perSheetKolomG
      ? 'Hasil pemeriksaan kolom G ("Kode Barang") di tiap sheet bulanan terhadap kode baku Stock GD MKS. INI sumber utama untuk pertanyaan per bulan: tiap temuan punya "lokasi" berisi sheet dan NOMOR BARIS tempat data diinput — sebutkan itu supaya bisa langsung dibetulkan (mis. "sheet Jun baris 332"). "status" per sheet: "bersih", "ada salah input", atau "belum ada input" (sheet belum diisi, bukan bersih). Pakai angka di sini untuk bagian per bulan, bukan dari "daftar".'
      : null,
    catatanPerBulan: (bulanDiminta || mintaPerBulan)
      ? 'Yang diperiksa: kode barang yang DIINPUT di transaksi tapi TIDAK ADA di daftar kode baku Stock GD MKS. Pembagian per bulan HANYA untuk transaksi — kesalahan penulisan di sheet Stock GD MKS sendiri tidak punya tanggal input, jadi tidak bisa dibagi per bulan; sampaikan ini kalau relevan. Kalau user menyebut satu bulan, sajikan "kodeTransaksiTidakAdaDiStok" untuk bulan itu saja, dan kalau "jumlahKode":0 katakan tegas bulan itu BERSIH. Kalau minta per bulan, sajikan "perBulan" urut dari bulan terlama; untuk angka di dalam tiap bulan WAJIB pakai "rincian" milik bulan itu, JANGAN memakai angka dari "daftar" (itu total seluruh periode, bukan per bulan). Bulan yang tidak muncul di "perBulan" berarti bersih.'
      : null,
...
// ==== Format penomoran invoice yang berlaku di cabang ====
// Ditetapkan langsung oleh Branch Manager, bukan disimpulkan dari pola data. Dipakai untuk
// memeriksa penulisan nomor faktur di TIGA sumber sekaligus: sales (Grand Data), revenue
// (Rev SUM), dan piutang (AR) — kesalahan ketik di satu sumber sering tidak ikut terlihat di
// sumber lain, jadi memeriksa satu tempat saja meninggalkan sisa.
// Susunan: kode (INV / R-MKS / R/MKS) + "/" + kode company (MKS atau CFN, khusus CFN memakai
// tanda hubung "INV-CFN") + "/" + TAHUN + "/" + bulan angka romawi + "/" + nomor urut.
// Tahun sengaja TIDAK dikunci ke 2026: revenue memuat pelunasan nota lama — terbukti ada
// pembayaran bernomor tahun 2021, 2023, 2024, dan 2025 di data hidup.
const INVOICE_ROMAWI = 'I{1,3}|IV|VI{0,3}|IX|XI{0,2}';
const FORMAT_INVOICE = [
  { bentuk: 'MKI borongan', company: 'MKI', jenis: 'Borongan — tidak bisa diberikan faktur pajak', contoh: 'INV/MKS/2026/I/001', re: new RegExp(`^INV/MKS/\\d{4}/(?:${INVOICE_ROMAWI})/\\d+$`) },
  { bentuk: 'MKI reguler', company: 'MKI', jenis: 'Invoice reguler', contoh: 'INV/MKS/2026/I/F-001', re: new RegExp(`^INV/MKS/\\d{4}/(?:${INVOICE_ROMAWI})/F-\\d+$`) },
  { bentuk: 'MKI faktur pajak', company: 'MKI', jenis: 'Invoice berfaktur pajak', contoh: 'INV/MKS/2026/I/FP-001', re: new RegExp(`^INV/MKS/\\d{4}/(?:${INVOICE_ROMAWI})/FP-\\d+$`) },
  // Retur punya DUA bentuk yang sama-sama sah, "R-MKS/" dan "R/MKS/" — ditegaskan Branch Manager.
  // Sebelum ini "R/MKS/" dilaporkan sebagai salah tulis dan sembilan retur Mei ikut tertuduh.
  { bentuk: 'MKI retur', company: 'MKI', jenis: 'Retur / pengembalian barang', contoh: 'R-MKS/2026/II/005', re: new RegExp(`^R-MKS/\\d{4}/(?:${INVOICE_ROMAWI})/\\d+$`) },
  { bentuk: 'MKI retur', company: 'MKI', jenis: 'Retur / pengembalian barang', contoh: 'R/MKS/2026/II/005', re: new RegExp(`^R/MKS/\\d{4}/(?:${INVOICE_ROMAWI})/\\d+$`) },
  { bentuk: 'CFN', company: 'CFN', jenis: 'Satu format untuk reguler DAN faktur pajak — CFN tidak mengenal borongan', contoh: 'INV-CFN/2026/VIII/126', re: new RegExp(`^INV-CFN/\\d{4}/(?:${INVOICE_ROMAWI})/\\d+$`) },
];
function bentukInvoiceSah(no) {
  const s = String(no || '').trim();
  for (const f of FORMAT_INVOICE) if (f.re.test(s)) return f;
  return null;
}
// Dugaan penyebab, murni mekanis (bentuk penulisan), bukan menebak maksud bisnisnya.
function dugaanSalahTulis(no) {
  const s = String(no || '').trim();
  const sebab = [];
  if (/\/\//.test(s)) sebab.push('ada garis miring ganda "//"');
  if (/[a-z]/.test(s)) sebab.push('ada huruf kecil');
  if (/\/F\d/i.test(s)) sebab.push('"F" tanpa tanda hubung — seharusnya "F-"');
  if (/\/fF-/i.test(s)) sebab.push('huruf F ganda "fF-"');
  if (/\.$/.test(s)) sebab.push('ada titik di akhir');
  if (/\/\./.test(s)) sebab.push('ada titik sebelum nomor');
  if (/\/\d+-\d+$/.test(s)) sebab.push('nomor urut memuat tanda hubung');
  // Alasan awalan hanya disebut kalau tidak ada sebab yang lebih spesifik — "R/MKS" sudah jelas
  // dijelaskan oleh aturan retur di atas, tidak perlu diulang dengan kalimat umum.
  if (!sebab.length && /^(?!INV\/MKS|INV-CFN|R-MKS|R\/MKS)[A-Z]+[/-]MKS/i.test(s)) sebab.push('awalan bukan "INV/MKS", "R-MKS", "R/MKS", atau "INV-CFN"');
...
function findInvoiceFormatIssues(message, allTransactions, paymentDetail, piutangDetail) {
  ...
  if (!bicaraInvoice || (!bicaraSalah && !bicaraPenulisan)) return null;
  // "Apa format penulisan invoice MKI?" itu pertanyaan penjelasan, bukan permintaan audit —
  // biarkan dijawab dari daftar format saja tanpa menyeret ratusan nomor ke dalam konteks.
  const mintaAudit = bicaraSalah || /carikan|cari|temukan|daftar|list|audit|periksa|cek|scan|sisir/.test(nMsg);
  if (!mintaAudit) {
    return { tipe: 'penjelasanFormat', formatResmi: FORMAT_INVOICE.map((f) => ({ bentuk: f.bentuk, company: f.company, jenis: f.jenis, contoh: f.contoh })) };
  }
  ...
  const catat = (no, sumber, info) => {
    const s = String(no || '').trim();
    if (!s) return;
    if (!kumpul.has(s)) kumpul.set(s, { noFaktur: s, sumber: new Set(), info: info || {} });
    kumpul.get(s).sumber.add(sumber);
  };
  const diperiksa = { sales: new Set(), revenue: new Set(), ar: new Set() };
  if (pakai.sales) {
    for (const t of allTransactions || []) {
      if (!t || !t.invoice) continue;
      diperiksa.sales.add(String(t.invoice).trim());
      if (!bentukInvoiceSah(t.invoice)) catat(t.invoice, 'SALES', { tanggal: t.tanggal, customer: t.customer, company: t.company, nilai: t.amount });
    }
  }
  if (pakai.revenue) {
    for (const r of paymentDetail || []) {
      if (!r || !r.noFaktur) continue;
      diperiksa.revenue.add(String(r.noFaktur).trim());
      if (!bentukInvoiceSah(r.noFaktur)) catat(r.noFaktur, 'REVENUE', { tanggal: r.tanggal, customer: r.customer, company: r.company, nilai: r.amount });
    }
  }
  if (pakai.ar) {
    for (const p of piutangDetail || []) {
      if (!p || !p.noFaktur) continue;
      diperiksa.ar.add(String(p.noFaktur).trim());
      if (!bentukInvoiceSah(p.noFaktur)) catat(p.noFaktur, 'AR', { tanggal: p.tanggal, customer: p.customer, company: p.company, nilai: p.nilaiSisa });
    }
  }
  ...
  const temuan = [...kumpul.values()]
    .map((x) => ({
      noFaktur: x.noFaktur,
      ditemukanDi: [...x.sumber],
      tanggal: x.info.tanggal || null,
      customer: x.info.customer || null,
      company: x.info.company || null,
      nilai: x.info.nilai != null ? x.info.nilai : null,
      dugaanMasalah: dugaanSalahTulis(x.noFaktur),
    }))
    .sort((a, b) => a.noFaktur.localeCompare(b.noFaktur));
  ...
  return {
    tipe: 'auditPenulisan',
    lingkup: adaLingkup ? lingkupDipakai : 'SEMUA (SALES, REVENUE, AR)',
    formatResmi: FORMAT_INVOICE.map((f) => ({ bentuk: f.bentuk, company: f.company, jenis: f.jenis, contoh: f.contoh })),
    jumlahNomorDiperiksa: { sales: diperiksa.sales.size, revenue: diperiksa.revenue.size, ar: diperiksa.ar.size },
    jumlahMenyimpang: temuan.length,
    menyimpangPerSumber: perSumber,
    temuan,
    catatan: temuan.length
      ? `Sumber yang disisir: ${lingkupDipakai.join(', ')}. Semua nomor di sini TIDAK cocok dengan format resmi mana pun. Sajikan dikelompokkan per sumber dan sebutkan "dugaanMasalah" tiap nomor supaya langsung bisa dibetulkan. Satu nomor bisa muncul di lebih dari satu sumber — itu ditulis di "ditemukanDi". Angka-angka sudah dihitung, jangan hitung ulang. JANGAN melaporkan sumber di luar yang disisir.`
      : `Sumber yang disisir: ${lingkupDipakai.join(', ')}. Tidak ada penulisan yang menyimpang di sumber itu. Katakan semuanya sudah sesuai format, dan sebut sumber mana yang diperiksa.`,
  };
}
...
function findDuplikasi(message, allTransactions, paymentDetail, piutangDetail) {
  ...
  if (semua || mintaPiutang) {
    ...
    // The DATE is what separates a real double entry from two legitimate billings that happen to
    // share a number. Ignoring it produced a false alarm on live data: INV/MKS/2026/VII/F-039
    // appears twice for the same customer at the same amount, but dated 8-Jul and 28-Jul — two
    // separate transactions, not a duplicate. Only same-invoice + SAME-DATE + same-amount is
    // strong evidence of a double entry.
    const dup = [...per.entries()]
      .filter(([, rows]) => rows.length > 1)
      .map(([noFaktur, rows]) => {
        const nilai = rows.map((r) => r.nilaiSisa);
        const tanggal = [...new Set(rows.map((r) => r.tanggal))];
        const nilaiSama = nilai.every((v) => v === nilai[0]);
        const tanggalSama = tanggal.length === 1;
        let jenis, dugaan, berisiko;
        if (tanggalSama && nilaiSama) {
          jenis = 'TAGIH_GANDA';
          berisiko = nilai[0] * (rows.length - 1);
          dugaan = 'Tanggal DAN nilai sama persis — kuat dugaan satu tagihan tercatat dua kali. Risiko: customer ditagih dobel.';
        } else if (!tanggalSama) {
          jenis = 'NOMOR_SAMA_TANGGAL_BEDA';
          berisiko = 0;
          dugaan = `DUPLIKASI NOMOR FAKTUR: nomor sama dipakai di tanggal berbeda (${tanggal.join(' & ')}). Ini tetap temuan yang harus ditindaklanjuti — saat menagih, dua tagihan berbeda memakai satu nomor mudah tertukar atau salah sasaran. Nilainya sendiri bukan tagihan dobel, tapi penomorannya wajib dibenahi.`;
        } else {
          jenis = 'SATU_FAKTUR_BANYAK_BARIS';
          berisiko = 0;
          dugaan = 'Tanggal sama, nilai berbeda — lazimnya satu faktur dengan beberapa baris barang, bukan duplikasi.';
        }
        return { noFaktur, customer: rows[0].customer, jenisTemuan: jenis, jumlahBaris: rows.length, tanggal, nilaiTiapBaris: nilai, totalTercatat: nilai.reduce((s, v) => s + v, 0), nilaiBerisikoTagihGanda: berisiko, dugaan };
      })
      .sort((a, b) => b.nilaiBerisikoTagihGanda - a.nilaiBerisikoTagihGanda);
    const tagihGanda = dup.filter((d) => d.jenisTemuan === 'TAGIH_GANDA');
    const nomorSama = dup.filter((d) => d.jenisTemuan === 'NOMOR_SAMA_TANGGAL_BEDA');
    hasil.piutangFakturGanda = {
      // Both of the first two ARE duplications and both must be reported; they differ only in
      // what the risk is — one bills twice, the other makes it easy to bill the wrong thing.
      totalKasusDuplikasi: tagihGanda.length + nomorSama.length,
      jumlahTagihGanda: tagihGanda.length,
      jumlahNomorSamaTanggalBeda: nomorSama.length,
      jumlahSatuFakturBanyakBaris: dup.length - tagihGanda.length - nomorSama.length,
      totalNilaiBerisikoTagihGanda: tagihGanda.reduce((s, d) => s + d.nilaiBerisikoTagihGanda, 0),
      daftar: dup,
    };
  }
...
function findTransactionMatches(message, allTransactions) {
  ...
  if (noteParts.length) {
    ...
    if (hitCustomers.length > 1) {
      // Do NOT leave this empty. Returning nothing here caused a real, serious failure: asked
      // about "Delta Sky Tech" (which exists twice, as "PT DELTA SKY TECH" and "PT. DELTA SKY
      // TECH"), transaksiRelevan came back empty, and rather than asking which one, Gemini filled
      // the vacuum — it invented an invoice number belonging to a different customer entirely, a
      // made-up invoice value, and a nine-item product list that does not exist. An empty field is
      // an invitation to fabricate, so hand over the REAL rows for every matching variant and let
      // the answer be grounded even while the ambiguity is flagged.
      matched = allTransactions.filter((tx) => hitCustomers.includes(tx.customer)).sort(byDateDesc);
      note =
        `Nama ini cocok ke BEBERAPA customer yang benar-benar ada di data: ${hitCustomers.join(', ')} — ejaannya mirip, bisa jadi orang/PT yang SAMA tapi tertulis beda di data, bisa juga memang beda. ` +
        'Baris di "transaksiRelevan" ini berisi transaksi dari SEMUA nama itu digabung — field "customer" tiap baris menunjukkan ejaan mana persisnya. ' +
        'WAJIB: sebutkan bahwa ada beberapa ejaan nama tersebut, pisahkan angkanya per ejaan (jangan diam-diam menggabung jadi satu angka seolah satu customer), dan tanyakan ke user apakah maksudnya salah satu saja atau keduanya digabung.';
    } else {
...
function findPiutangByCustomer(message, piutangDetail) {
  ...
  if (hits.length > 1) {
    // Same reasoning as findTransactionMatches: hand over the real rows per variant instead of an
    // empty field, so an ambiguous name can't become a fabricated invoice number.
    return {
      customerCandidatesAmbiguous: hits,
      perEjaan: hits.map((h) => {
        const inv = piutangDetail.filter((p) => p.customer === h);
        return { customer: h, jumlahInvoice: inv.length, totalSisaPiutang: inv.reduce((s, p) => s + p.nilaiSisa, 0), invoices: inv };
      }),
      catatan: `Nama ini cocok ke BEBERAPA ejaan customer yang benar-benar ada: ${hits.join(', ')} — bisa jadi customer yang SAMA tertulis beda. Angka per ejaan ADA di "perEjaan" (pakai itu, JANGAN mengarang), sebutkan terpisah per ejaan dan tanyakan ke user maksudnya yang mana atau digabung.`,
    };
  }
  const hit = hits[0];
...
// "Berapa persentase customer aktif sejak Mei?" — a real question that came back "data tidak
// tersedia" even though every ingredient was present: transactions carry a date and a customer, so
// who bought within a period is directly countable. Answers both the count and the share of the
// full-year customer base, plus which of them are repeat buyers within that same period.
function findCustomerAktifPeriode(message, allTransactions) {
  if (!allTransactions || !allTransactions.length) return null;
  const nMsg = normText(message);
  const wants = /customer\s+aktif|pelanggan\s+aktif|persentase\s+(customer|pelanggan)|(customer|pelanggan).*(sejak|semenjak|mulai\s+bulan)|berapa\s+persen.*(customer|pelanggan)/.test(nMsg);
  if (!wants) return null;

  // Period start: an explicit month if named ("sejak Mei"), otherwise the current month.
  const mm = extractMonthMention(message);
  const now = nowMakassar();
  const mulai = mm ? new Date(mm.year, mm.month - 1, 1) : new Date(now.getFullYear(), now.getMonth(), 1);
  const labelBulan = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];

  const semua = new Set();
  const aktif = new Map(); // customer -> set of invoices within the period
  for (const tx of allTransactions) {
    if (!tx.customer) continue;
    semua.add(tx.customer);
    const d = parseFlexibleDate(tx.tanggal);
    if (!d || d < mulai) continue;
    if (!aktif.has(tx.customer)) aktif.set(tx.customer, new Set());
    if (!tx.isRetur && tx.invoice) aktif.get(tx.customer).add(tx.invoice);
  }
  const jumlahAktif = aktif.size;
  const jumlahSemua = semua.size;
  const berulang = [...aktif.values()].filter((s) => s.size > 1).length;

  return {
    periode: `sejak ${labelBulan[mulai.getMonth()]} ${mulai.getFullYear()} s.d. hari ini`,
    totalCustomerSepanjangTahun: jumlahSemua,
    customerAktifDiPeriode: jumlahAktif,
    persenAktif: jumlahSemua ? Math.round((jumlahAktif / jumlahSemua) * 1000) / 10 : 0,
    customerTidakAktifDiPeriode: jumlahSemua - jumlahAktif,
    persenTidakAktif: jumlahSemua ? Math.round(((jumlahSemua - jumlahAktif) / jumlahSemua) * 1000) / 10 : 0,
    customerBelanjaLebihDariSekali: berulang,
    catatan:
      '"Aktif" = customer yang punya minimal SATU transaksi dalam periode itu; pembaginya adalah seluruh customer yang pernah bertransaksi sepanjang tahun berjalan. Semua angka dan persentase SUDAH dihitung — jangan hitung ulang manual. ' +
      'Ini murni hitungan dari data transaksi; sistem TIDAK punya penanda kejadian bisnis apa pun (mis. kenaikan harga), jadi kalau user mengaitkan periode ini dengan sebuah peristiwa, sampaikan bahwa angkanya dihitung dari rentang tanggalnya saja, bukan dari peristiwa itu.',
  };
}
...
async function runSync(env) {
  ...
  let kodeBakuSync = null; // dipakai langkah 1b di bawah
  try {
    const kodeMentah = [];
    ...
    kodeBakuSync = new Set(kodeMentah.map((x) => String(x.kode).trim().toUpperCase()));
  ...
  // 1b) Kolom G ("Kode Barang") di tiap sheet bulanan Jan-Des — tempat tim benar-benar
  // menginput. Diperiksa terhadap daftar kode baku Stock GD MKS, dan yang disimpan hanya
  // temuannya beserta SHEET dan NOMOR BARIS-nya, supaya MIRA bisa menunjuk tepat di mana harus
  // dibetulkan. Grand Data tidak bisa memberi itu: baris di sana bukan tempat data diinput.
  // Satu sheet yang gagal dibaca tidak menggagalkan yang lain, dan sheet yang tiba-tiba kosong
  // padahal sebelumnya berisi dianggap gangguan — temuan lamanya dipertahankan, bukan diganti
  // "bersih" yang palsu.
  if (kodeBakuSync) {
    try {
      const lamaRaw = await env.SHEET_CACHE.get('data:auditKolomG');
      const lama = lamaRaw ? JSON.parse(lamaRaw) : null;
      const diperiksaPerSheet = {};
      const temuan = [];
      const gagalSheet = [];
      for (const [sheet, gid] of Object.entries(GIDS_BULANAN)) {
        try {
          const rows = parseCsv(await (await fetch(csvExportUrl(PERFORMANCE_SHEET_ID, gid))).text());
          const header = (rows[0] || []).map((h) => (h || '').trim());
          // Kolom G menurut arahan Branch Manager — tetap dicocokkan dengan judulnya supaya
          // kolom yang bergeser tidak diam-diam memeriksa isi kolom lain.
          const iKode = header[6] === 'Kode Barang' ? 6 : header.indexOf('Kode Barang');
          if (iKode < 0) { gagalSheet.push(`${sheet}: kolom Kode Barang tidak ditemukan`); continue; }
          let jumlah = 0;
          const temuanSheet = [];
          for (let i = 1; i < rows.length; i++) {
            const r = rows[i] || [];
            const mentahKode = String(r[iKode] ?? '');
            const k = mentahKode.trim();
            if (!k) continue;
            jumlah += 1;
            const terdaftar = kodeBakuSync.has(k.toUpperCase());
            const rapi = k === mentahKode && k === k.toUpperCase();
            if (terdaftar && rapi) continue;
            temuanSheet.push({
              sheet,
              baris: i + 1,
              kode: mentahKode,
              masalah: !terdaftar ? 'tidak terdaftar di Stock GD MKS' : k !== mentahKode ? 'ada spasi di awal atau akhir kode' : 'memuat huruf kecil',
              invoice: String(r[3] ?? '').trim(),
              tanggal: String(r[2] ?? '').trim(),
              customer: String(r[5] ?? '').trim(),
            });
          }
          const sebelumnya = lama && lama.diperiksaPerSheet ? lama.diperiksaPerSheet[sheet] : 0;
          if (jumlah === 0 && sebelumnya > 0) {
            gagalSheet.push(`${sheet}: terbaca kosong padahal sebelumnya ${sebelumnya} baris — hasil lama dipertahankan`);
            diperiksaPerSheet[sheet] = sebelumnya;
            temuan.push(...(lama.temuan || []).filter((t) => t.sheet === sheet));
            continue;
          }
          diperiksaPerSheet[sheet] = jumlah;
          temuan.push(...temuanSheet);
        } catch (e) {
          gagalSheet.push(`${sheet}: ${String(e)}`);
          if (lama) {
            diperiksaPerSheet[sheet] = (lama.diperiksaPerSheet || {})[sheet] || 0;
            temuan.push(...(lama.temuan || []).filter((t) => t.sheet === sheet));
          }
        }
      }
      await env.SHEET_CACHE.put('data:auditKolomG', JSON.stringify({ diperiksaPerSheet, temuan, gagalSheet, waktu: new Date().toISOString() }));
      summary.sources.auditKolomG = { ok: true, temuan: temuan.length, gagal: gagalSheet.length };
    } catch (err) {
      summary.sources.auditKolomG = { ok: false, error: String(err) };
    }
  }
...
async function handleChat(request, env) {
  ...
  const [
    ...
    customerActivityRaw, targetHarianRaw, stockKodeMentahRaw, auditKolomGRaw, lastSync,
  ] = await Promise.all([
    ...
    env.SHEET_CACHE.get('data:auditKolomG'),
  ...
  // "Berapa jumlah customer kita?" matched none of these keywords, so the field came back null and
  // MIRA answered with wilayah/transaction counts instead — the plainest possible question about
  // customers went unanswered. Counting phrasings added.
  const wantsCustomerInsights = wantsSaran || /frekuensi|churn|tidak aktif|jarang (ber)?belanja|paling sering (ber)?belanja|loyal|repeat ?order|by\s*sales|by\s*frekuensi|belanja terbesar|pembelian terbesar|paling banyak (ber)?belanja|(jumlah|berapa|total|banyak)\s+(customer|pelanggan)|customer\s+(aktif|unik|kita)|pelanggan\s+(aktif|unik|kita)/.test(nMsgTopic);
  ...
  const context = {
    ...
    cekPenulisanKodeProduk: findKodeProdukIssues(message, stockKodeMentahRaw ? JSON.parse(stockKodeMentahRaw) : null, allTransactions, auditKolomGRaw ? JSON.parse(auditKolomGRaw) : null),
...
- "CEK DUPLIKASI"/"ada yang ganda?"/"dobel"/"kembar" → WAJIB jawab dari "cekDuplikasi" SAJA. Tujuannya MENCEGAH TAGIH GANDA dan PENAGIHAN SALAH SASARAN, jadi susun jawaban per kategori dan DAHULUKAN yang berisiko tinggi: (1) "piutangFakturGanda" — faktur tercatat >1 kali di piutang. Sebutkan "totalKasusDuplikasi" sebagai angka utama, lalu RINCI per jenis (WAJIB dibedakan, jangan disamakan): "TAGIH_GANDA" (tanggal DAN nilai sama persis) = customer berisiko ditagih dobel, sebutkan "jumlahTagihGanda" + "totalNilaiBerisikoTagihGanda". "NOMOR_SAMA_TANGGAL_BEDA" (nomor faktur sama, tanggal beda) = TETAP DUPLIKASI dan tetap wajib ditindaklanjuti — sebutkan "jumlahNomorSamaTanggalBeda" beserta nomor+tanggal-tanggalnya, risikonya tagihan tertukar/salah sasaran karena satu nomor dipakai dua tagihan; TAPI jangan sebut nilainya sebagai "tagihan dobel". "SATU_FAKTUR_BANYAK_BARIS" (tanggal sama, nilai beda) = lazim, sebutkan sebagai catatan ringan saja. Kesalahan nyata yang pernah terjadi: MIRA menyebut 4 faktur "kuat dugaan tagih ganda Rp47 juta" padahal tanggalnya berbeda — salah karena menyamakan dua jenis ini; SELALU baca "jenisTemuan" dan "tanggal" dulu. (2) "invoiceBedaTanggal" — nomor invoice sama muncul di tanggal berbeda; yang "customerBerbeda":true itu PALING BAHAYA (bisa menagih orang yang salah), sebutkan duluan lengkap dengan tanggal+customer tiap kejadiannya. (3) "pembayaranKembar" — pembayaran identik tercatat dua kali. Sebutkan "jumlahKasus" tiap kategori; kalau 0 katakan kategori itu bersih. WAJIB sampaikan sebagai TEMUAN YANG PERLU DICEK, bukan vonis salah — sebagian bisa wajar (mis. satu faktur beberapa baris, atau entri koreksi). Angka sudah dihitung, jangan hitung ulang.
...
- FORMAT PENOMORAN INVOICE RESMI CABANG (ditetapkan langsung oleh Branch Manager — ini FAKTA, bukan tebakan).
  SUSUNANNYA: kode (INV untuk penjualan, atau R-MKS / R/MKS untuk retur) + garis miring + KODE COMPANY (MKS untuk MKI; khusus CFN ditulis menyatu "INV-CFN") + garis miring + TAHUN + garis miring + BULAN dalam angka ROMAWI (I, II, III, ... XII) + garis miring + NOMOR URUT.
  TAHUN TIDAK SELALU 2026 — di Revenue wajar muncul tahun lama karena itu pelunasan nota lama (di data hidup ada 2021, 2023, 2024, 2025). Nomor bertahun lama BUKAN kesalahan.
  MKI punya empat bentuk, dan huruf di nomor urutnya MENENTUKAN JENISNYA:
   • "INV/MKS/2026/I/001" (tanpa huruf) = BORONGAN — jenis ini TIDAK BISA diberikan faktur pajak.
   • "INV/MKS/2026/I/F-001" = invoice REGULER.
   • "INV/MKS/2026/I/FP-001" = invoice BERFAKTUR PAJAK.
...
- CEK PENULISAN KODE PRODUK ("cek kode produk yang salah", "kode barang yang tidak sesuai di Stock GD MKS", "kode produk salah tulis di sales", "ada kode barang yang tidak terdaftar?", "salah penginputan kode barang", "salah penginputan kode barang bulan Juli", "salah input kode barang per bulan") → WAJIB dari "cekPenulisanKodeProduk". Untuk pertanyaan PER BULAN, per sheet, atau yang menyebut "kolom G" ("cek kolom G di setiap bulan", "salah input kode barang bulan Juli"): WAJIB utamakan "kolomGSheetBulanan" — tiap sheet bulanan membawa "status" (bersih / ada salah input / belum ada input) dan tiap temuan membawa "lokasi" berisi NOMOR BARIS di sheet itu. Sebutkan sheet dan barisnya (mis. "sheet Jun baris 332, kode KCO6002, invoice INV/MKS/2026/VI/055") supaya langsung bisa dibetulkan. "belum ada input" artinya sheet itu belum diisi — JANGAN sebut bersih. Baca juga "catatanKolomG" dan "catatanPerBulan". Ada DUA bagian, sajikan terpisah: (A) "penulisanDiStok" — kesalahan penulisan di sheet Stock GD MKS sendiri, dipisah "pastiSalah" (mekanis dan terbukti: huruf kecil, spasi, tanda baca di ujung, kode ganda — sebutkan baris sheet dan usulan perbaikannya) dan "perluDicek" (mencurigakan tapi BELUM TENTU salah — sampaikan sebagai perlu dicek, bukan tuduhan). (B) "kodeTransaksiTidakAdaDiStok" — kode yang dipakai di transaksi tapi tidak terdaftar di Stock GD MKS, beserta jumlah baris, nilai, contoh invoice, dan "kemungkinanMaksudnya". Untuk (B) WAJIB jelaskan akibatnya: transaksi itu tidak ikut terhitung di penjualan per produk, kategori kabel, dan pencarian pembeli. "kemungkinanMaksudnya" hanyalah USULAN dari kemiripan penulisan — pakai kata "kemungkinan", jangan pastikan. Baca "lingkup": kalau user hanya menyebut stok, jangan menyinggung transaksi seolah sudah diperiksa, dan sebaliknya. JANGAN menyatakan sebuah kode salah hanya karena bentuknya berbeda dari kebanyakan kode — banyak keluarga kode yang memang sah (KSFO028, CR1N001, DGT0001, TO2W01A, ADIS006-MC). Semua sudah dihitung, jangan hitung ulang.
...
- RANKING/PENJUALAN PRODUK, 4 field beda: "produk terlaris" tanpa bulan (kumulatif) → "topProduk" (byAmount=rupiah, byQty=unit, top-20, tiap item punya "nama" — SELALU sebutkan nama, jangan cuma kode SKU). "Produk terlaris BULAN X" → "topProdukPerBulan" ("bulan" konfirmasi periode, byAmount/byQty top-20 khusus itu; null padahal sebut bulan = kata kunci ranking tak terdeteksi). "Penjualan/sales kode X" ATAU membandingkan BEBERAPA kode sekaligus (mis. "bandingkan KSFO113 dan KSFO128") → "produkSalesDetailPerKode": "perKode" berisi SATU ENTRI PER KODE yang disebut user — WAJIB jawab SEMUA kode di situ, DILARANG bilang salah satu kode "tidak punya rincian bulanan"/"tidak tersedia" kalau kodenya ada di "perKode" (ini kesalahan nyata yang pernah terjadi berulang). Tiap entri punya "totalSepanjangTahun" dan "perBulan" (YYYY-MM, sudah lengkap Jan s.d. bulan berjalan). Bulan bernilai 0 = MEMANG tidak ada penjualan bulan itu, sebut "0", JANGAN sebut "data belum tercatat". Diminta perbandingan antar PERIODE (mis. Jan-April vs Mei-Agustus) → jumlahkan sendiri bulan-bulan yang diminta dari "perBulan", datanya sudah lengkap; kalau diminta persentase perubahan, hitung dari dua angka periode itu ((periode2-periode1)/periode1*100) dan sebutkan dasar hitungannya. "Kabel 1 core"/"di atas 1 core"/bandingkan kategori → "kabelKategoriCore" ("kategoriDibandingkan"=array per kategori disebut, tiap entri: kategori/totalPenjualanGabungan/totalQtyGabungan=agregat PENJUALAN gabungan semua kode/daftarProduk=daftar tiap kode dengan kode+nama+harga+STOK ("stokTotal"/"stokMKI"/"stokCFN") masing-masing; "totalKodeProduk":0 = tak ada yang cocok). Pertanyaan soal STOK kategori kabel (mis. "kabel 1 core beserta stoknya") → WAJIB baca "stokTotal"/"stokMKI"/"stokCFN" per item di "daftarProduk", JANGAN malah menjawab pakai "totalPenjualanGabungan"/"totalQtyGabungan"/harga (itu data PENJUALAN, beda pertanyaan) — kalau user cuma minta stok, cukup sebutkan stoknya, jangan melebar ke angka penjualan yang tidak diminta.
